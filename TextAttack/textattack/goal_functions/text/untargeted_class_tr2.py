...
class UntargetedClassTr(TextToTextGoalFunction):
    ...
    def __init__(self, *args, ground_max_score=None, max_min_score=None, dif_score=None, alpha=1, thr=1, num_class=1, cw=False, **kwargs):
    ...
    def _is_goal_complete(self, model_output, _):
        ...
        if self.num_class==1:
            ...
            # skip 
            if self.iter==1 and max_index!=self.ground_truth_output:
                    return True
                
            elif max_index!=self.ground_truth_output:
            ...
            if self.iter==1: 
                if max_index!=self.ground_truth_output and max_index1!=self.ground_truth_output:
                    return True
                elif max_index!=self.ground_truth_output and max_index1==self.ground_truth_output:
                    self.attack1 = True
                elif max_index==self.ground_truth_output and max_index1!=self.ground_truth_output:
                    self.attack = True
                elif max_index==self.ground_truth_output and max_index1==self.ground_truth_output:
            ...
            # attack both       
            if self.attack and self.attack1: 
                if max_index!=self.ground_truth_output and max_index1!=self.ground_truth_output:
                    if self.ground_max_score:
            ...
            # attack classifier two
            elif self.attack1:
                if max_index1!=self.ground_truth_output:
                    if self.ground_max_score:
                        return logits1[self.ground_truth_output] < self.ground_max_score and 1-self.tr_score <= self.thr
                    elif self.max_min_score:
                        return logits1[max_index1] > self.max_min_score and 1-self.tr_score <= self.thr
                    elif self.dif_score:
                        dif = logits1[max_index1] - logits1[self.ground_truth_output]
                        return dif > self.dif_score and 1-self.tr_score <= self.thr
                    else:
                        return 1-self.tr_score <= self.thr
                else:
    ...
    def _get_score(self, model_output, _):
        ...
        # class_score = 1 - softmax[self.ground_truth_output] #logits[self.ground_truth_output]

        class_score = 1 - torch.sigmoid(logits[self.ground_truth_output])

        if self.cw:
            logit_sorted = logits.topk(2)
            max_logit = logit_sorted.values[logit_sorted.indices!=self.ground_truth_output][0]

            class_score = 1 - torch.sigmoid(logits[self.ground_truth_output] - max_logit)

        if self.num_class==1 or self.num_class==0:
            return self.alpha * class_score + (1-self.alpha) * tr_score
        elif self.num_class==2:
            logits1 = model_output[2]
            class_score1 = 1 - torch.sigmoid(logits1[self.ground_truth_output])
            return self.alpha * (class_score+class_score1)/2 + (1-self.alpha) * tr_score
